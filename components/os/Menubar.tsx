...
export default function Menubar() {
  ...
  const handleMenuAction = (action?: string) => {
    ...
    if (action === 'random') {
      ...
      // do nothing — just closes menu, bringing focus to desktop
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3">
        <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" style={{ color: 'var(--accent)', opacity: 0.8 }}>
        ...
        {Object.keys(MENUS).map((menu) => (
          <div key={menu} className="relative">
            <button
              ...
              style={{
                color: activeMenu === menu ? 'var(--text-primary)' : 'var(--text-muted)',
                  ...
                  {MENUS[menu].map((item, i) => (
                    item.label.startsWith('──') ? (
                      ...
                      <button
                        ...
                        onMouseEnter={(e) => {
                          (e.currentTarget as HTMLElement).style.background = 'rgba(124,106,255,0.15)';
      ...
      <div className="flex items-center gap-3">
        ...
        {/* Wifi */}
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ color: 'var(--text-muted)', opacity: 0.55 }}>
        ...
        {/* Battery */}
        <svg width="18" height="10" viewBox="0 0 22 11" fill="none" style={{ opacity: 0.55 }}>
          <rect x="0.5" y="0.5" width="18" height="10" rx="2" stroke="var(--text-muted)" strokeWidth="1" />
          <rect x="2" y="2" width="14" height="7" rx="1" fill="var(--accent)" opacity="0.7" />
          <rect x="19" y="3.5" width="2.5" height="4" rx="1" fill="var(--text-muted)" opacity="0.5" />
        </svg>

        {/* Date + Time */}
        <div
          className="flex items-center gap-2"
          style={{ color: 'var(--text-muted)', fontSize: '11px', fontFamily: 'var(--font-mono)' }}
        >
          <span>{date}</span>
